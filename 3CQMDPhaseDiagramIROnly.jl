...
cutoff = 1000
lam, v, m_lam, hx, hy, c = 0.001, 0, 969/cutoff, 4.2, 3, 1750000/cutoff^3
lamy, vy, m_lamy, lamxy = 8.0, 0, 900.0/cutoff, -1 #8.0, 0, 900.0/cutoff, -0.6
...
#PARALLEL
T_min, T_max = 5/cutoff, 180/cutoff
mu_min, mu_max = 1/cutoff, 450/cutoff #450/cutoff
N_T, N_mu = 175, 450 #220
T_array, mu_array = LinRange(T_min, T_max, N_T), LinRange(mu_min, mu_max, N_mu)
...
name = string("QMDphaseDiagramIRhx",string(hx),"hy",string(hy),"mu_min",string(mu_min*cutoff),"Nmu",string(N_mu))
mkdir(name)
println(name)
...
@time begin
    @inbounds Threads.@threads for i = 1:N_T
        @inbounds Threads.@threads for j = 1:N_mu
            ...
            #s = solve(prob, CVODE_BDF(), saveat=k)
...
for i = 1:N_T
    ...
    for j = 1:N_mu
        sol_export = []
        append!(sol_export, sol[Int64((i-1)*N_mu + j)][3].u[end])
